...
#define PRINT(a) fprintf(stderr, "%s %s\n", a, "Passed")
...
int main(int argc, char *argv[])
{
    ...
    }
    {
        ...
        std::vector<bpt::value_t> values(end - start + 1);
        assert(tree.search_range(&key1, key2, (bpt::value_t *)&values[0], end - start + 1) == end - start + 1);
        ...
        bool next;
        assert(tree.search_range(&key1, key2, (bpt::value_t *)&values[0], end - start + 100) == end - start + 1);
        assert(tree.search_range(&key1, key2, (bpt::value_t *)&values[0], end - start + 100, &next) == end - start + 1);
        assert(next == false);
        PRINT("SearchRangeSuccess");
        assert(tree.search_range(&key1, key1, (bpt::value_t *)&values[0], end - start + 1) == 1);
        assert(tree.search_range(&key1, key1, (bpt::value_t *)&values[0], end - start + 1, &next) == 1);
        assert(next == false);
        assert(tree.search_range(&key2, key2, (bpt::value_t *)&values[0], end - start + 1) == 1);
        assert(tree.search_range(&key2, key2, (bpt::value_t *)&values[0], end - start + 1, &next) == 1);
        assert(next == false);
        PRINT("SearchRangeSameKey");
        assert(tree.search_range(&key2, key1, (bpt::value_t *)&values[0], end - start + 1) == -1);
        assert(tree.search_range(&key1, key2, (bpt::value_t *)&values[0], end - start) == end - start);
        assert(tree.search_range(&key1, key2, (bpt::value_t *)&values[0], end - start, &next) == end - start);
        assert(next == true);
